...
line_link link_MF, link_MY;

void *TrainModelThread_MF(void *id) {
...
void *TrainModelThread_MY(void *id) {
    long long edge_count = 0, last_edge_count = 0;
    unsigned long long next_random = (long long)id;
    real *error_vec_u = (real *)calloc(vector_size, sizeof(real));
    real *error_vec_v = (real *)calloc(vector_size, sizeof(real));
    ...
    while (1)
    {
        ...
        link_MY.train_sample(error_vec_u, error_vec_v, alpha, gsl_rng_uniform(gsl_r), gsl_rng_uniform(gsl_r), next_random);        
...
void TrainModel() {
    ...
    clock_t start = clock();
    printf("Training MF process:\n");
    for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, TrainModelThread_MF, (void *)a);
    for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
    printf("\n");
    alpha = starting_alpha;
    edge_count_actual = 0;
    printf("Training MY process:\n");
    for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, TrainModelThread_MY, (void *)a);
    for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
    printf("\n");
