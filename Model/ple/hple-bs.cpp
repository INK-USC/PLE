...
void TrainModel() {
    ...
    sprintf(file_name, "%smention_type_%s.txt", file_path, boostrap_iter);
    link_MY.init(file_name, &node_M, &node_Y, negative);
    
    sprintf(file_name, "%sfeature_type_%s.txt", file_path, boostrap_iter);
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-boostrap", argc, argv)) > 0) strcpy(boostrap_iter, argv[i + 1]);
    ...
    sprintf(file_path, "/srv/data/wenqihe/%s/reduce_label_noise/", data);
    sprintf(output_path, "/srv/data/wenqihe/%s/reduce_label_noise/Results/", data);
    lr = starting_lr;
    TrainModel();
    return 0;
}