...
void *train_miniBatch_thread(void *id)
{
    ...
    for (int k = begin; k != end; k++, T++)
    {
        ...
        int u = feature_set[k];
        link_FY.train_miniBatch_sg(error_vec_u, error_vec_v, lr, next_random, u);
    }
...
void TrainModel() {
    ...
    if (mode == 'm')
    {
        ...
        {
            std::random_shuffle(feature_set.begin(), feature_set.end());
    ...
    sprintf(file_name, "%semb_hple_bipartite_feature.txt", output_path);
    node_F.output(file_name, binary);
    sprintf(file_name, "%semb_hple_bipartite_type.txt", output_path);
