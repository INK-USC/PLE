...
int main(int argc, const char * argv[]) {
    if ( argc != 6 ) {
        ...
        printf( "usage: %s -INDIR -EMBED_SIZE -LR -MAX_ITER -N\n", argv[0] );
    }
    else {
        ...
        snprintf(filename, sizeof(filename), "%s/feature.txt", indir);
        feature_count = count_lines(filename);
        snprintf(filename, sizeof(filename), "%s/type.txt", indir);
        type_count = count_lines(filename);
        snprintf(filename, sizeof(filename), "%s/mention.txt", indir);
        ...
        /* Save matrix A and B*/
        snprintf(filename, sizeof(filename), "%s/Results/warp_A.txt", indir);
        print_matrix(filename, A, feature_count, embed_size);
        snprintf(filename, sizeof(filename), "%s/Results/warp_B.txt", indir);
