...
class Predicter_useFeatureEmb:
    ...
    def predict_types_for_mention_maximum(self, mention_id, _threshold, candidate):
    ...
    def predict_types_for_mention_topDown(self, mention_id, _threshold, candidate):
        _type_size = self._embs_type._node_size
        _emb_mention = self.get_mention_embedding(mention_id)
    ...
    def predict_types_for_mention_topk(self, mention_id, _threshold, candidate):
...
def clean(indir, outdir, _method, _emb_mode, _predict_mode, _sim_func, _threshold):
    ...
    with open(os.path.join(outdir + '/mention_type_' + _method + '_' + _emb_mode + '.txt'), 'w') as g:
        ...
        for mention_id in candidates:
            ...
            cnt += 1
            if cnt % 10000 == 0:
                sys.stdout.write('{0} mentions done\r'.format(cnt))
                sys.stdout.flush()
        g.close()
    print cnt, 'mentions cleaned.'
